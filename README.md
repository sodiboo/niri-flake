...
For debugging (primarily development of this flake i guess), there is
also `programs.niri.finalConfig` which is always a string (or null)
and represents the final rendered config file that will end up in your
config directory.
...
>
> This does not apply to `programs.niri.config` and
> `programs.niri.extraConfig` as those are inherently
> version-agnostic.
>
