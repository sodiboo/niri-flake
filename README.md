...
I have a binary cache for this flake's outputs. `niri.cachix.org` hosts builds of `niri-stable` and `niri-unstable` for `nixos-unstable` and `nixos-25.05`. It contains builds for both `x86_64-linux` and `aarch64-linux`, built natively on GitHub Actions runners.
